...
void start_dhcp_server(void) 
{
	...
	IP4_ADDR(&info.gw, 192, 168, 88, 2); //ESP acts as router, so gw addr will be its own addr
 ...
 ************************************************/
static void WIFI_vInitCommon(wifi_mode_t enWifiMode)
{
...
void vTaskWifiHandler( void * pvParameters )
{
    ...
    for(;;)
	{
        ...
        if(enWifiMode == WIFI_MODE_STA)
        {
            ...
            EventBits_t bits = xEventGroupWaitBits(APP_event_group, APP_event_WIFI_AP_CONNECTED_BIT, pdFALSE, pdFALSE, (60000 / portTICK_PERIOD_MS)/*min*/ * 5);
            if (bits & APP_event_WIFI_AP_CONNECTED_BIT) 
            {
				enWifiMode = WIFI_MODE_AP;
				stop_OTA_webserver(OTA_server);
                ...
                err=nvs_set_i8(my_handle,"wifi_mode",enWifiMode);
