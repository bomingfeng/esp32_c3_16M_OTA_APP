...
static esp_err_t YAPOF3_parser_get_scan_code(ir_parser_t *parser, uint32_t *address, uint32_t *command, bool *repeat)
{
    esp_err_t ret = ESP_FAIL;
    uint32_t addr = 0;
    uint32_t cmd = 0;
    ...
    YAPOF3_parser_t *YAPOF3_parser = __containerof(parser, YAPOF3_parser_t, parent);
        ...
        YAPOF3_parse_repeat_frame(YAPOF3_parser);
            ...
            *repeat = false;
            ...
            YAPOF3_parser->last_address = addr;
            YAPOF3_parser->last_command = cmd;
            ret = ESP_OK;
        }
    return ret;
}
